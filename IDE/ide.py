...
def crater():
	...
	return a
def get_canvas():
	cnvs = Canvas(root,bg='orange',width = 1200,height = 700)
	cnvs.pack(side=LEFT,pady=0)
	#cnvs.configure(bg="blue")
	return cnvs
def file_name_label():
	a = Label(root,text = "File Name",bg='orange')	
	a.pack()
	return a
cnvs = get_canvas();	
...
def file_create_button():
	btn = Button(root,width = 10,bg='yellow',text = "Open or Save",command = write_file)
	btn.pack(padx = 10)
	return btn
def opn():
	os.system('./creater')
def file_formatter_button():
	btn = Button(root,width = 15,bg='yellow',text = 'Open File Creater',command = opn)
	btn.pack(padx = 10)
	return btn
...
def sinp():
	...
	inf.close()
def add_input_btn():
	btn = Button(root,width=10,bg='yellow',text="save input",command = sinp)
	btn.pack()
	return btn
def run():
	nm = file_name.get()
...
cnvs.create_window(750,20,window = save_btn)
run_btn = run_f_btn()
cnvs.create_window(860,20,window = run_btn)
a = Text(root,width = 45,height = 12,bg = 'blue',foreground='white')
a.pack()
cnvs.create_window(835,589,window = a)
